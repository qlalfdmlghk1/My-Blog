...
### Commit — 2026-09-21 12:20

- Message: `Feat: 로고를 액센트 계열 보라 버전으로 교체`
- Issue: (미생성 — github-issue-draft.md)
- Jira: 미사용

**변경 요약**

- `public/logo.png` · `src/app/icon.png`: 복숭아색 → 연보라 면 + 진한 보라 (같은 마스코트, 168×168)
- `SiteHeader` 주석: "팔레트 밖 색" 근거를 "액센트와 같은 계열" 로 정정

**결정 로그**

- 복숭아색은 화면에서 그 색만 어디에도 안 속해 떠 보였다. 액센트(`--accent` 계열)와 맞추면 로고·버튼이 한 브랜드로 읽힌다
- purple 카테고리 배지와의 구분은 색이 아니라 형태(24px 얼굴 vs 작은 알약)로 한다
